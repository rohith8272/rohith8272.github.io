...
2. **Docker setup**
   We will use the docker container that has the complete environment already setup for us.
   ```bash
         docker run --rm \
      --device /dev/fuse \
      --cap-add SYS_ADMIN \
...
the PX4 cloud simulation available on dronesim with the following settings:
...
My custom plugin repository contains other examples such as mavlink data plotting and map interactions.
